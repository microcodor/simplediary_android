...
public class MentionModelImp implements MentionModel {
    ...
    public void cacheSave(int groupType, Context context, String response) {
        if (NewFeature.CACHE_MESSAGE_MENTION) {
            switch (groupType) {
                case Constants.GROUP_RETWEET_TYPE_ALL:
                    SDCardUtil.put(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"message/mention", "所有微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
                    break;
                case Constants.GROUP_RETWEET_TYPE_FRIENDS:
                    SDCardUtil.put(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"message/mention", "关注人的微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
                    break;
                case Constants.GROUP_RETWEET_TYPE_ORIGINWEIBO:
                    SDCardUtil.put(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"message/mention", "原创微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
                    break;
                case Constants.GROUP_RETWEET_TYPE_ALLCOMMENT:
                    SDCardUtil.put(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"message/mention", "所有评论" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
                    break;
                case Constants.GROUP_RETWEET_TYPE_FRIEDNSCOMMENT:
                    SDCardUtil.put(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"message/mention", "关注人的评论" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
                    break;
            }
    ...
    public void cacheLoad(int groupType, Context context, OnMentionFinishedListener onDataFinishedListener) {
        if (NewFeature.CACHE_MESSAGE_MENTION) {
            ...
            switch (groupType) {
                case Constants.GROUP_RETWEET_TYPE_ALL:
                    response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"message/mention", "所有微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
                    break;
                case Constants.GROUP_RETWEET_TYPE_FRIENDS:
                    response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"message/mention", "关注人的微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
                    break;
                case Constants.GROUP_RETWEET_TYPE_ORIGINWEIBO:
                    response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"message/mention", "原创微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
                    break;
            }
    ...
    public void cacheLoad(int groupType, Context context, OnCommentFinishedListener onCommentFinishedListener) {
        if (NewFeature.CACHE_MESSAGE_MENTION) {
            ...
            switch (groupType) {
                case Constants.GROUP_RETWEET_TYPE_ALLCOMMENT:
                    response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"message/mention", "所有评论" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
                    break;
                case Constants.GROUP_RETWEET_TYPE_FRIEDNSCOMMENT:
                    response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"message/mention", "关注人的评论" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
                    break;
            }
            if (response != null) {
