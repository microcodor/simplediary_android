...
import com.wxdroid.simplediary.entity.list.TokenList;
...
public class UserModelImp implements UserModel {
    ...
    public void getUserDetailList(final Context context, final OnUserListRequestFinish onUserListRequestFinish) {
        String jsonstring = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + "/simplediary", "登录列表缓存.txt");
        if (jsonstring == null && AccessTokenKeeper.readAccessToken(context).isSessionValid()) {
            cacheCurrentOuthToken(context);
        }
        TokenList tokenList = TokenList.parse(SDCardUtil.get(context, SDCardUtil.getSDCardPath() + "/simplediary", "登录列表缓存.txt"));
        if (tokenList == null) {
    ...
    public void cacheSave_statuslist(int groupType, Context context, String response) {
        ...
        switch (groupType) {
            case Constants.GROUP_MYWEIBO_TYPE_ALL:
                SDCardUtil.put(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的全部微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
                break;
            case Constants.GROUP_MYWEIBO_TYPE_ORIGIN:
                SDCardUtil.put(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的原创微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
                break;
            case Constants.GROUP_MYWEIBO_TYPE_PICWEIBO:
                SDCardUtil.put(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的图片微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
                break;
        }
    }
    ...
    public void cacheLoad_statuslist(int groupType, Context context, OnStatusListFinishedListener onStatusListFinishedListener) {
        ...
        switch (groupType) {
            case Constants.GROUP_MYWEIBO_TYPE_ALL:
                response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的全部微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
                break;
            case Constants.GROUP_MYWEIBO_TYPE_ORIGIN:
                response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的原创微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
                break;
            case Constants.GROUP_MYWEIBO_TYPE_PICWEIBO:
                response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的图片微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
                break;
        }
        if (response != null) {
    ...
    public void cacheSave_user(Context context, String response) {
        SDCardUtil.put(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的基本信息" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
    }

    @Override
    public void cacheLoad_user(Context context, OnUserDetailRequestFinish onUserDetailRequestFinish) {
        String response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的基本信息" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
        if (response != null) {
    ...
    public void cacheSave_userlist(int groupType, Context context, String response) {
        switch (groupType) {
            case FOLLOWERS_LISTS:
                SDCardUtil.put(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的粉丝列表" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
                break;
            case FRIENDS_LISTS:
                SDCardUtil.put(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的关注列表" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
                break;
        }
    }
    ...
    public void cacheLoad_userlist(int groupType, Context context, OnUserListRequestFinish onUserListRequestFinish) {
        ...
        switch (groupType) {
            case FOLLOWERS_LISTS:
                response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的粉丝列表" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
                break;
            case FRIENDS_LISTS:
                response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的关注列表" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
                break;
        }
        if (response != null) {
    ...
    public void cacheCurrentOuthToken(Context context) {
        ...
        Gson gson = new Gson();
        SDCardUtil.put(context, SDCardUtil.getSDCardPath() + Constants.FileDir, "登录列表缓存.txt", gson.toJson(tokenList));
    }
