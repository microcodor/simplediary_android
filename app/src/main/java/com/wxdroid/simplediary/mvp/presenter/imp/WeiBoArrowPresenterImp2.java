...
import com.wxdroid.simplediary.mvp.model.StatusListModel;
...
import com.wxdroid.simplediary.mvp.model.imp.StatusListModelImp;
import com.wxdroid.simplediary.mvp.presenter.WeiBoArrowPresent2;
    ...
    public void updateLocalFile(String weiboGroup, int position) {
        ...
        switch (weiboGroup) {
            case Constants.DELETE_WEIBO_TYPE1:
                response = SDCardUtil.get(mContext, SDCardUtil.getSDCardPath() + Constants.FileDir+"home", "全部微博" + AccessTokenKeeper.readAccessToken(mContext).getUid() + ".txt");
                break;
            case Constants.DELETE_WEIBO_TYPE2:
                response = SDCardUtil.get(mContext, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的全部微博" + AccessTokenKeeper.readAccessToken(mContext).getUid() + ".txt");
                break;
            case Constants.DELETE_WEIBO_TYPE3:
                response = SDCardUtil.get(mContext, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的原创微博" + AccessTokenKeeper.readAccessToken(mContext).getUid() + ".txt");
                break;
            case Constants.DELETE_WEIBO_TYPE4:
                response = SDCardUtil.get(mContext, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的图片微博" + AccessTokenKeeper.readAccessToken(mContext).getUid() + ".txt");
                break;
            case Constants.DELETE_WEIBO_TYPE5:
                response = SDCardUtil.get(mContext, SDCardUtil.getSDCardPath() + Constants.FileDir+"home", "好友圈" + AccessTokenKeeper.readAccessToken(mContext).getUid() + ".txt");
                break;
            case Constants.DELETE_WEIBO_TYPE6:
                response = SDCardUtil.get(mContext, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的收藏" + AccessTokenKeeper.readAccessToken(mContext).getUid() + ".txt");
                ...
                if (favoriteList != null && favoriteList.favorites.size() > 0 && position < favoriteList.favorites.size()) {
                    ...
                    SDCardUtil.put(mContext, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的收藏" + AccessTokenKeeper.readAccessToken(mContext).getUid() + ".txt", new Gson().toJson(favoriteList));
                }
                return;
        }
        ...
        if (statusList != null && statusList.statuses.size() > 0 && position < statusList.statuses.size()) {
            ...
            switch (weiboGroup) {
                case Constants.DELETE_WEIBO_TYPE1:
                    SDCardUtil.put(mContext, SDCardUtil.getSDCardPath() + Constants.FileDir+"home", "全部微博" + AccessTokenKeeper.readAccessToken(mContext).getUid() + ".txt", new Gson().toJson(statusList));
                    break;
                case Constants.DELETE_WEIBO_TYPE2:
                    SDCardUtil.put(mContext, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的全部微博" + AccessTokenKeeper.readAccessToken(mContext).getUid() + ".txt", new Gson().toJson(statusList));
                    break;
                case Constants.DELETE_WEIBO_TYPE3:
                    SDCardUtil.put(mContext, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的原创微博" + AccessTokenKeeper.readAccessToken(mContext).getUid() + ".txt", new Gson().toJson(statusList));
                    break;
                case Constants.DELETE_WEIBO_TYPE4:
                    SDCardUtil.put(mContext, SDCardUtil.getSDCardPath() + Constants.FileDir+"profile", "我的图片微博" + AccessTokenKeeper.readAccessToken(mContext).getUid() + ".txt", new Gson().toJson(statusList));
                    break;
                case Constants.DELETE_WEIBO_TYPE5:
                    SDCardUtil.put(mContext, SDCardUtil.getSDCardPath() + Constants.FileDir+"home", "好友圈" + AccessTokenKeeper.readAccessToken(mContext).getUid() + ".txt", new Gson().toJson(statusList));
                    break;
            }
        }
    }
