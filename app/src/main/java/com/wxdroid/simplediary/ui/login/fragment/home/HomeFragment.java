package com.wxdroid.simplediary.ui.login.fragment.home;
...
import com.wxdroid.simplediary.mvp.presenter.HomeFragmentPresent;
import com.wxdroid.simplediary.mvp.presenter.WeiBoArrowPresent2;
import com.wxdroid.simplediary.mvp.presenter.imp.HomeFragmentPresentImp;
...
import com.wxdroid.simplediary.ui.login.fragment.home.groupwindow.GroupPopWindow;
import com.wxdroid.simplediary.ui.login.fragment.home.groupwindow.IGroupItemClick;
import com.wxdroid.simplediary.ui.login.fragment.home.weiboitem.HomeHeadView;
...
//                popupWindow.showAtLocation(mRecyclerView, Gravity.CENTER, 0, 0);
        ...
        mHeaderAndFooterRecyclerViewAdapter.getHeaderView().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ToastUtil.showShort(getActivity(),"head click");
            }
        });
    ...
    public void updateListView(ArrayList<Status> statuselist) {
        ...
        if (statuselist!=null&&statuselist.size()>0){
            for (Status status:statuselist){
                if (status.pic_urls!=null&&status.pic_urls.size()>0){
                    LogUtil.d("updateListView","size:"+status.user.screen_name);
                    LogUtil.d("updateListView","size:"+status.pic_urls.size());
                    LogUtil.d("updateListView","urls:"+ new Gson().toJson(status.pic_urls));
                    LogUtil.d("updateListView*********************************");
                }
            }
        }
        mDatas = statuselist;
