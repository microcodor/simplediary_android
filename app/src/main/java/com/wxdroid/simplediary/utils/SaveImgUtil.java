 ...
 * 默认图片保存目录是/sdcard/Android/data/com.wxdroid.simplediary.file/Pictures/weibo
 * 最好设置一下fileName
...
public class SaveImgUtil {
    ...
    private static final String FOLDER_NAME = "SimpleDiaryPic";
    ...
    public void saveImage(File imgFile, Bitmap bitmap) {
        // 首先保存图片
        File appDir = new File(Environment.getExternalStorageDirectory(), "simpleDiaryImg");
        if (!appDir.exists()) {
            appDir.mkdir();
        }
